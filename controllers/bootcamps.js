...
exports.createBootcamp =asyncHandler( async (req, res, next) => {
 ...
 if(publishBootcamp && req.user.role !== 'admin'){
    return next(new ErrorResponse(`the user with id of ${req.user.id} has already published`, 400))
 }
...
exports.updateBootcamp =asyncHandler(async (req, res, next) => {
        ...
        if(bootcamp.user.toString() !== req.user.id && req.user.role !== 'admin'){
            return next(new ErrorResponse(`User ${req.user.id} is not authorized to update bootcamp`))
        }
...
exports.deleteBootcamp =asyncHandler(async (req, res, next) => {
        ...
        //make sure user user is bootcamp owner
        if(bootcamp.user.toString() !== req.user.id && req.user.role !== 'admin'){
            return next(new ErrorResponse(`User ${req.user.id} is not authorized to delete bootcamp`))
        }
        bootcamp.remove()
...
exports.bootcampPhotoUpload =asyncHandler(async (req, res, next) => {
   ...
   if(!req.files){
    ...
    //check filesize
    if(file.size = process.env.MAX_FILE_UPLOAF){
        return next(new ErrorResponse('Please upload the image less than 100000', 400))
    }
